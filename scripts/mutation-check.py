...
MUTATIONS: list[tuple[str, str, str, str, str]] = [
    ...
    ("card", "jcs.rs", "ties not rounded to even", "let (digits, exp) = even_on_tie(x.abs(), digits, exp);", "let (digits, exp) = (digits, exp);"),
    ("card", "jcs.rs", "unsafe integers rounded", "if !(-MAX_SAFE_INTEGER..=MAX_SAFE_INTEGER).contains(&i) {", "if false {"),
    ("card", "jcs.rs", "depth unlimited", "if depth > MAX_DEPTH {", "if false {"),
    # --- a2a-gov-extension ---------------------------------------------------------
